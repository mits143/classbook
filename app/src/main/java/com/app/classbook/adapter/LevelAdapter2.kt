...
import com.app.classbook.model.response.RoyaltyIncomeModel

class LevelAdapter2(
    internal var context: Context,
    val dataList: ArrayList<RoyaltyIncomeModel>,
    val clickListener: (RoyaltyIncomeModel) -> Unit
) :
    RecyclerView.Adapter<LevelAdapter2.ViewHolder>() {

    //this method is returning the view for each item in the list
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): LevelAdapter2.ViewHolder {
        val v = LayoutInflater.from(parent.context).inflate(R.layout.cell_level, parent, false)
        return ViewHolder(v)
    }

    //this method is binding the data on the list
    override fun onBindViewHolder(holder: LevelAdapter2.ViewHolder, position: Int) {
    ...
    class ViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
        ...
        fun bindItems(data: RoyaltyIncomeModel, clickListener: (RoyaltyIncomeModel) -> Unit) {
