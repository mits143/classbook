...
class ActivityCartPresenter : ActivityCartView.MainPresenter {

    var context: Context? = null
    var mainView: ActivityCartView.MainView? = null

    @NonNull
    var disposable: Disposable? = null

    constructor(context: Context?, mainView: ActivityCartView.MainView?) {
        ...
        if (NetWorkConection.isNEtworkConnected(context!!)) {
            disposable = ApiClient.instance
                ...
                .subscribe({ listResponse ->
                    ...
                    when (val responseCode = listResponse.code()) {
                        200, 201, 202, 204 -> {
                            mainView!!.onSuccessRemoveCart(listResponse)
                        }
                        400, 401, 500 -> {
                            mainView!!.onError(responseCode)
                        }
